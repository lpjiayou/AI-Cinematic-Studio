...
def validate_a14b_runtime_attestation(value, *, backend_profile=None, process_identity=None, execution_config=None):
    ...
    _require(value["schemaVersion"] == A14B_RUNTIME_ATTESTATION_SCHEMA and value["capabilityMode"] == A14B_CAPABILITY_MODE,
    ...
    _require(facts["comfyuiCommit"] == COMFYUI_COMMIT and facts["comfyuiVersion"] == COMFYUI_COMMIT
        and facts["compilerIdentity"] == A14B_COMPILER_IDENTITY and facts["templateRef"] == A14B_TEMPLATE_REF,
        "A14B runtime code/compiler/template mismatch")
    _require(facts["requiredNodes"] == list(A14B_REQUIRED_NODES)
        and canonical(facts["nodeInputContracts"]) == canonical(A14B_NODE_INPUT_CONTRACTS), "A14B node inputs changed")
