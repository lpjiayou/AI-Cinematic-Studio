...
2. create a private staging directory next to the target so final rename stays on one
   filesystem;
3. create the lifecycle and episode-production databases only inside staging;
...
7. run the existing read-only scanner against staging and require exactly one current
   production run;
8. write a secret-free receipt and SHA-256 inventory, fsync where supported, then
   atomically rename staging to the canonical target;
...
- authenticated Creator Public API reads match the receipt;
