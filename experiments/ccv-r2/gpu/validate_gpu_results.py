#!/usr/bin/env python3
"""Independently validate a completed CCV-R2 G2 result root."""
...
from execute_gpu_experiment import (
...
def validate(result_root: Path) -> tuple[str, str]:
    ...
    require(receipt_sha == EXPECTED_RECEIPT_SHA256, "source preparation receipt changed")
    ...
    require(isinstance(rows, list) and len(rows) == EXPECTED_RUN_COUNT, "result ledger must contain 45 rows")
    ...
    require(failures.get("schemaVersion") == "ACS-CCV-R2-G2-FAILURE-LEDGER-1", "failure ledger schema mismatch")
    ...
    require(inventory.get("schemaVersion") == "ACS-CCV-R2-G2-RESULT-INVENTORY-1", "result inventory schema mismatch")
    ...
    require(execution_receipt.get("preparationReceiptSha256") == receipt_sha, "execution receipt preparation binding mismatch")
...
def main() -> int:
    ...
    try:
        ...
        print("CCV_R2_G2_RESULT_VALIDATION=PASS")
        print("OUTPUT_COUNT=45")
        ...
        print("CCV_R2_G2_RESULT_VALIDATION=FAIL")
        print(f"ERROR={exc}")
        return 1
