#!/usr/bin/env python3
"""Execute the frozen CCV-R2 G3 remediation matrix through local ComfyUI.

The runner is bound to the G3-G1 readiness receipt and a G3-G2 authorization document.
...
GOVERNANCE_CHECKPOINT = "ACS-CCV-R2-G3-G2-GPU-EXECUTION"
EXPECTED_RECEIPT_SHA256 = "e39ac4a8c3ddbf1f26571b295bcb00da7ff6b499acd49e8ad47291726bbbc5e4"
EXPECTED_INVENTORY_SHA256 = "20878b06608af6310cc1e60648d5b5e59a05cc6aa43475feb9ff3f9ae1e62845"
EXPECTED_RUN_COUNT = 51
...
def validate_preparation(preparation_root: Path) -> None:
    validator = Path(__file__).resolve().parents[1] / "preflight" / "validate_g3_preparation.py"
    require(validator.is_file(), f"G3-G1 validator missing: {validator}")
    ...
    require(result.returncode == 0, "G3-G1 preparation validation failed")
...
def verify_authorization(
    ...
    required = {
        "schemaVersion": "ACS-CCV-R2-G3-G2-EXECUTION-AUTHORIZATION-1",
        "gpuExecutionAuthorized": True,
        "governanceCheckpoint": GOVERNANCE_CHECKPOINT,
        "readinessSha256": EXPECTED_RECEIPT_SHA256,
...
def verify_png_metadata(path: Path, expected: dict[str, Any]) -> str:
    ...
    require("ccvR2G3" in text, f"PNG ccvR2G3 metadata missing: {path}")
    actual = json.loads(text["ccvR2G3"])
    require(actual == expected, f"PNG ccvR2G3 metadata mismatch: {path}")
...
def initial_failure_ledger(receipt_sha: str, authorization_sha: str) -> dict[str, Any]:
    return {
        "schemaVersion": "ACS-CCV-R2-G3-G2-FAILURE-LEDGER-1",
        "experimentId": "acs-ccv-r2",
        "state": "EMPTY",
        "readinessSha256": receipt_sha,
        "authorizationSha256": authorization_sha,
...
def record_failure(
    ...
    event_id = f"G3-F{len(failures['events']) + 1:04d}"
    event = {
        "eventId": event_id,
        "runId": request["runId"],
        "blindLabel": request["blindLabel"],
...
def finalize(
    ...
    require(len(completed) == EXPECTED_RUN_COUNT, "cannot finalize before 51 completed runs")
    output_rows = []
    ...
    inventory = {"schemaVersion": "ACS-CCV-R2-G3-G2-RESULT-INVENTORY-1", "entries": output_rows}
    ...
    review = {
        "schemaVersion": "ACS-CCV-R2-G3-G2-BLIND-REVIEW-PACKAGE-1",
    ...
    execution_receipt = {
        "schemaVersion": "ACS-CCV-R2-G3-G2-EXECUTION-RECEIPT-1",
        ...
        "sourceReadinessHost": receipt.get("host"),
        "claims": {
            "gpuExecutionAuthorized": True,
...
def execute(
    ...
    *,
    ...
    require(receipt_path.is_file() and inventory_path.is_file(), "G3-G1 preparation controls missing")
    receipt_sha, _ = sha256_file(receipt_path)
    inventory_sha, _ = sha256_file(inventory_path)
    require(receipt_sha == EXPECTED_RECEIPT_SHA256, "G3-G1 readiness digest is not authorized")
    require(inventory_sha == EXPECTED_INVENTORY_SHA256, "G3-G1 inventory digest is not authorized")
    if validate_first:
        validate_preparation(preparation_root)
    receipt = load_json(receipt_path)
    requests = receipt.get("requests")
    require(isinstance(requests, list) and len(requests) == EXPECTED_RUN_COUNT, "readiness receipt must contain 51 requests")
...
def main() -> int:
    ...
    try:
        ...
        print("CCV_R2_G3_G2_GPU_EXECUTION=PASS")
        print("RUN_COUNT=51")
        ...
        print("CCV_R2_G3_G2_GPU_EXECUTION=FAIL_CLOSED")
        print(f"ERROR={exc}")
        return 1
