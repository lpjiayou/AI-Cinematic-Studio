#!/usr/bin/env python3
"""Standard-library tests for the fail-closed G3-G2 executor."""
...
import execute_g3_gpu_experiment as runner
...
def png_with_metadata(metadata: dict[str, object]) -> bytes:
    ...
    return b"".join(
        [
            ...
            chunk(b"tEXt", b"ccvR2G3\x00" + json.dumps(metadata, sort_keys=True).encode("latin-1")),
...
class FakeComfyClient:
    ...
    def wait(self, prompt_id: str) -> dict[str, str]:
        ...
        metadata = payload["extra_data"]["extra_pnginfo"]["ccvR2G3"]
...
class G3G2ExecutionTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temp = Path(tempfile.mkdtemp(prefix="ccv-r2-g3-g2-test-"))
            ...
            for seed in SEEDS:
                for shot in SHOTS:
                    ...
                    payload = {
                        ...
                        "extra_data": {"extra_pnginfo": {"ccvR2G3": metadata}},
                    }
        ...
        for weight in ("0p30", "0p45"):
            for seed in SEEDS:
                counter += 1
                run_id = f"sweep__weight-{weight}__seed-{seed}__shot-04_back_turning"
                blind_label = f"G3B{counter:03d}"
                planned = f"outputs/back_turning_sweep/ip-{weight}/seed-{seed}/04_back_turning.png"
                metadata = {
                    "runId": run_id,
                    "blindLabel": blind_label,
        ...
        receipt = {
            "schemaVersion": "ACS-CCV-R2-G3-READINESS-1",
            "experimentId": "acs-ccv-r2",
            "protocolVersion": "g3-rcr-v1",
            "state": "G3_G1_PREPARATION_COMPLETE_NO_GPU_EXECUTION",
            "requests": requests,
            "software": {"comfyUiCommit": "mock", "ipAdapterNodeCommit": "mock"},
            "host": {"gpu": {"name": "mock"}},
        }
        self.receipt_path = self.preparation / "g3-readiness.json"
    ...
    def authorization(self, result_root: Path, **changes: object) -> Path:
        value = {
            ...
            "readinessSha256": runner.EXPECTED_RECEIPT_SHA256,
            "preparationInventorySha256": runner.EXPECTED_INVENTORY_SHA256,
            "expectedRunCount": 51,
            "maximumQueueCount": 51,
            "maximumInFlight": 1,
    ...
    def test_51_runs_are_strictly_sequential_and_validate(self) -> None:
        ...
        self.assertEqual(client.submissions, 51)
        ...
        self.assertEqual(ledger["counts"]["succeeded"], 51)
        validator = importlib.import_module("validate_g3_gpu_results")
    ...
    def test_terminal_failure_is_ledgered_once_and_stops(self) -> None:
        ...
        self.assertEqual(ledger["counts"], {"expected": 51, "queued": 3, "succeeded": 2, "failed": 1})
    ...
    def test_completed_resume_never_requeues(self) -> None:
        ...
        self.assertEqual(first.submissions, 51)
