...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `R-CORE-ARCH-001` | M1 AI Director、M3 Script Studio、M5 Series Director 与 Creator Server 的四个活动生产文件直接导入 V4 public provider boundary，跳过 V5 相邻层；同一模式跨三个里程碑复用且被现有 Contract Test 正向固化，导致仓库实现与 `Application → V5 → V4` 强制方向冲突。 | 高：破坏架构一致性，使 V4 DTO、错误与执行策略传播到 Application，并削弱未来 V5 治理与替换能力 | 高：四个活动文件已存在且模式重复，当前没有全仓静态守卫 | 依照 ADR-0006 建立 V5-owned Text Generation capability boundary；在精确 G1 白名单内迁移四个 Application 文件；增加所有 `apps/**/*.py` 禁止导入 V4 的 AST/文本守卫；保持 Prompt、候选、HTTP、V4 Adapter 和持久化行为不变。完成条件：G1 全部专项/回归/架构/Git 门禁通过，远端验证后由 Project Lead 单独接受并在后续治理 checkpoint 更新风险状态。 | 缓解中（`MITIGATING`） | Project Lead / Architecture Owner `蔺鹏` | 当前证据：四个生产直连、无正式例外、现有测试固化该依赖；监控触发：任一 `apps/**/*.py` 出现 `services.v4_platform`；关闭证据：生产导入 `0`、V5→V4 映射测试和 Full Core 回归 PASS、Remote SHA 等于 Local SHA、Owner Acceptance | `ACS-ARCH-R1-V5-TEXT-GENERATION-G1` Owner Review | `2026-08-13` | `ADR-0006`; `V5_TEXT_GENERATION_CAPABILITY_CONTRACT`; `ACS-ARCH-R1-V5-TEXT-GENERATION-G0` |
