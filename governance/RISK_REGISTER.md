...
| `R-K2-BOOT-011` | 原 K2 durable lineage 未找到；若用测试 Ref、默认空库、直接 SQL、非原子部分写入或把历史技术证据挂到新 run，新的 root 将不可审计且可能被误称为恢复。 | 严重：权威血缘失真、重复 root、错误 P1/发布结论 | 中：Project Lead 已批准重新引导，但实现与主机执行尚未完成 | ADR-0010 冻结一次性 Operator Application：显式新目录、同盘 staging、V5 public boundaries、无测试依赖、restart/read-only scan、secret-free receipt、原子 rename、重复 apply 拒绝，并在 `ROOTS_READY` 停止。 | 缓解中（`MITIGATING / BLOCKING LINEAGE`） | Project Lead / Architecture Owner `蔺鹏` | 触发：canonical 目标已存在仍写入、出现直接 SQL/测试 import、缺 receipt/digest、M6/P1 被自动推进；缓解证据：bootstrap targeted/full tests、远端同树、主机只读扫描和 Public API exact match | Canonical bootstrap G0→G1 | `2026-08-21` | `ADR-0010`; `K2_CANONICAL_LINEAGE_BOOTSTRAP_CONTRACT` |
