...
为准；技术独立审查不能替代 ruleset 要求的合格 GitHub Reviewer。

| 风险编号 | 风险描述 | 影响 | 概率 | 缓解措施 | 状态 | 责任人 | 触发条件 / 验证证据 | 目标日期 / 事件 | 最近复核日期 | 关联事项 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `R-REPO-GOV-012` | Core `main` ruleset ID `20544466` 的修复前快照为 `0` approvals，且缺少 linear/stale/latest-push/thread/strict 与 `Integration Tests`；该历史缺口已被 Owner-approved 单人运营配置取代。当前残余风险不是配置字段缺失，而是行为验证未完成，且 PR #11 作者为唯一账号、reviews=`[]`，没有合格非作者满足 `1 approval`。若把配置合规误写为运营关账或可合并，可能诱发绕过。 | 高：Core 配置已能表达所需门禁，但审批容量不足会永久阻塞当前作者 PR；以自动化替代平台审批将破坏主线审计 | 高：PR #11 当前明确为 `0/1` 且单账号无非作者 Reviewer；受控负向/正向行为验证未完成 | 保持 ruleset `20544466` 的一名非作者批准、stale/latest-push/thread、strict、linear/deletion/non-fast-forward、五项 required checks 和 bypass=`[]`；提供一个合格非作者账号取得 latest-push approval；完成受控负向/正向行为验证。Repository General settings 可继续显示三种 merge method，但结论必须限定为有效 `main` ruleset squash-only。 | 缓解中（`MITIGATING / CONFIGURATION-CONFORMING / PARTIALLY VERIFIED / REVIEWER CAPACITY BLOCKED`） | Repository Governance Owner `蔺鹏` | 当前 API 证据：approvals=`1`；allowed merge methods=`[squash]`；strict=`true`；`do_not_enforce_on_create=false`；Markdown、Documentation Links、Unit Tests、Contract Tests、Integration Tests 均 `integration_id=15368`；linear/deletion/non-fast-forward；bypass=`[]`。关闭需行为验证和一次具有真实非作者 approval 的正向对照；自动化/独立技术审查不计为 approval。 | Core reviewer provisioning + operational behavior closeout | `2026-08-25` | `BRANCH_PROTECTION`; `GIT_WORKFLOW 4A`; `ACS-K2-002-GOV-RB1` |
...
| `R-K2-GOV-017` | `ACS-K2-002-GOV-RB1` 要求合格 GitHub approval、完整检查和线性 squash merge。Core ruleset 现已在 Owner-approved 单人运营范围内配置合规，但 PR #11 author=`lpjiayou`、reviews=`[]`；作者不能自批，现有单账号无法提供必需的 `1 approval`。若以自动化/独立技术审查替代平台 approval，或使用 bypass，K2-002 候选会越过正式合并条件。 | 高：K2-002 的 Owner 决定、候选 SHA/tree 与主线接受证据链失真；不直接授权媒体、Provider/GPU 或发布 | 高：当前精确状态为 `0/1` 且无合格非作者账号；ruleset 没有 bypass | 绑定精确候选 SHA/tree；保持 ruleset `20544466` 当前五项 strict checks、latest-push/thread/linear 与 bypass=`[]`；提供一个合格非作者 GitHub Reviewer 并取得 latest-push `1/1` approval；独立技术审查仅作为证据，不计入平台审批；满足全部条件后使用有效 `main` ruleset 的 squash path 并核验合并后 Core `main`/Actions。 | 开放（`OPEN / BLOCKING K2-002 MAIN MERGE / CONFIGURATION FIX COMPLETE`） | Project Lead / Repository Governance Owner `蔺鹏` | 当前证据：Core configuration=`CONFORMING / PARTIALLY VERIFIED`；PR #11=`0/1 / MERGE BLOCKED`；Repository General settings 仍为 merge/squash/rebase，但有效 `main` ruleset allowed methods=`[squash]`。关闭证据：`ACS-K2-002-GOV-RB1` 第 9 节在精确候选上满足，且 approval 来自真实非作者、无 bypass。 | PR #11 eligible reviewer + merge gate | `2026-08-25` | `ACS-K2-002-GOV-RB1`; `ADR-0014`; `BRANCH_PROTECTION`; `GIT_WORKFLOW 4A`; `R-REPO-GOV-012` |
