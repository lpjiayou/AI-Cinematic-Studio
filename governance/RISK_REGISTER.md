...
| `R-K2-TRANS-013` | 复用 K2-001 root、数据库、refs、候选或 ADR-0011 例外会把 K2-002 错接到历史验证 lineage；把仓库写入授权、客户端声明或外部 v1.4 文件状态当成 Script Owner acceptance，也会污染来源权威。无 durable receipt/M5 binding 时执行多域注册仍会生成不可恢复的半套 roots。 | 严重：独立项目 provenance 失真并可能越权 dispatch | 中：正式 durable registration 尚未实现；v1.4 已通过双层来源/rebase 账本降低文档混淆 | 保留 K2-001 原路径、Core v1.3 和 v1 机器账本为只读历史；v2 必须同时记录 exact uploaded source、Core predecessor 和 repository-reviewed rebase digests。`importedByRef` 只能由认证 service credential 注入且不得表述为已解析自然人/RBAC。正式确认须解析可信 Owner approval；Project/Series/Episode registration 必须具备 durable receipt、重启幂等、部分写入恢复和 M5 v2 EpisodePlanItem binding。 | 缓解中（`MITIGATING / V1.4 LINEAGE SPLIT RECORDED`） | Project Lead / Architecture Owner `蔺鹏` | 出现 K2-001 ref、旧数据库路径、transferred exception、仓库 ingest 冒充内容接受、无 receipt 多域写入或未绑定 M5 source 即触发；当前证据为 `ACS-K2-002-SCRIPT-RB2` 与 versioned v1/v2 ledgers。关闭仍需可信 Script approval、durable canonical receipt、M5 consumer 成功与 authenticated exact-match。 | K2-002 roots gate | `2026-08-26` | `ADR-0014`; `ACS-K2-002-SCRIPT-RB2`; `K2-001 ARCHIVE` |
