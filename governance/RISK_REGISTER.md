...
| `R-CORE-SEC-003` | Creator Public HTTP/API v1 当前没有调用方认证并直接接受请求中的 `workspaceRef`；若在此状态下开放非回环监听，任何可达客户端都能选择工作区并调用已开放的 M1–M6 表面。 | 严重：可能造成跨工作区读取或写入，并把本地开发接口误用为可部署接口 | 高：认证控制当前缺失，且请求作用域由客户端字段直接决定 | 按 ADR-0007/AUTH-W1 实施服务端 Bearer 身份、凭据到唯一 workspace 映射、客户端 scope 拒绝、非回环 public-only 监听和双仓 Gate C；责任人：Project Lead / Architecture Owner `蔺鹏`。完成条件：全量测试、凭据泄露检查、双进程隔离测试、远端一致性全部通过。 | 缓解中（`MITIGATING`） | Project Lead / Architecture Owner `蔺鹏` | 触发条件：无认证 public v1、客户端可传 `workspaceRef`、非回环监听暴露 internal 路由；转监控证据：AUTH-W1 Gate C 及双仓 remote verify；关闭需另行安全复核，AUTH-W1 不得直接关闭 | `ACS-AUTH-W1` Gate C | `2026-08-17` | `ADR-0007`; `CREATOR_PUBLIC_API_AUTHENTICATION_AND_WORKSPACE_ISOLATION_CONTRACT` |
