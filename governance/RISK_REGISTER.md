...
仓库治理现状以 2026-08-25 远端快照、[`BRANCH_PROTECTION`](BRANCH_PROTECTION.md)
和 [`ACS-K2-002-GOV-RB1`](ACS-K2-002-NON-GPU-PREPRODUCTION-REBASELINE.md)
为准；Core 当前 ruleset 不要求 GitHub Reviewer，但精确候选独立技术审查仍是强制
治理证据。Frontend 的两名平台批准要求不受本 Core 决定影响。

| 风险编号 | 风险描述 | 影响 | 概率 | 缓解措施 | 状态 | 责任人 | 触发条件 / 验证证据 | 目标日期 / 事件 | 最近复核日期 | 关联事项 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `R-REPO-GOV-012` | Core `main` ruleset ID `20544466` 的修复前快照虽为 `0 approvals`，却缺少 linear/stale/latest-push/thread/strict 与 `Integration Tests`；其后的 `1 approval` 中间配置又因单账号形成 `0/1` capacity blocker。两者均已被当前 Owner-approved 单人运营零批准精确例外取代。当前残余风险是平台无人工批准隔离、职责集中，以及受控行为验证尚未完成；不得把配置合规误写为无审查、运营关账或自动可合并。 | 高：错误解释零批准可能削弱主线审计；但 Pull Request、独立技术审查、五项 strict checks、thread、linear/squash、no force/delete/bypass 仍提供补偿控制 | 中：零批准是显式 Owner 决定，PR #11 不再有 Reviewer capacity blocker；受控负向/正向行为验证仍未完成 | 保持 ruleset `20544466` 的 approvals=`0`、dismiss-stale/latest-push/extra-approval=`false`、thread、strict、linear/deletion/non-fast-forward、五项 required checks 和 bypass=`[]`；每个精确候选保留独立技术审查证据；完成适用的受控负向/正向行为验证；Core 增加运营账号或审批模型变化时重新决策。Repository General settings 的三种方法结论仍必须限定为有效 `main` ruleset squash-only。 | 缓解中（`MITIGATING / CONFIGURATION-CONFORMING / PARTIALLY VERIFIED / ZERO-APPROVAL EXCEPTION ACTIVE`） | Repository Governance Owner `蔺鹏` | 远端设置页保存后复读：approvals=`0`；dismiss-stale/latest-push/extra-approval=`false`；thread/strict/linear/deletion/non-fast-forward 生效；五项 required checks 仍显示 GitHub Actions source；allowed methods=`[squash]`；bypass=`[]`。本轮变更后的 API integration ID 未独立复读；关闭仍需行为验证，平台“缺少审批”负向项在该例外下为 `N/A`。 | Core operational behavior closeout + operator-count review | `2026-08-25` | `BRANCH_PROTECTION`; `GIT_WORKFLOW 4B`; `ACS-K2-002-GOV-RB1` |
| `R-REPO-GOV-016` | Frontend `main-protection-v1` ruleset ID `21413134` 已 active/effective，配置了两名批准、stale/latest-push/thread、linear+squash-only、三项 strict checks 且无 bypass；但合格 Reviewer 容量不足，受控负向/正向行为验证尚未完成，`delete_branch_on_merge=false`。若把配置存在或当前分支清洁误当作运营关账，可能造成 PR 永久阻塞、例外绕行或未来短期分支再次堆积。 | 高：技术门禁虽已配置，但审批可用性、执行有效性和持续分支卫生尚无完整证据 | 高：Reviewer 容量与行为验证缺口当前存在；自动删除明确关闭 | 补足至少两名可服务任一作者的合格非作者 Reviewer；验证直接推送/force/delete、缺审批、失败检查和未解决 thread 均被阻塞，并以正常受保护 PR 作正向对照；决定启用 `delete_branch_on_merge` 或记录等价受控清理。 | 缓解中（`MITIGATING / CONFIGURED BUT NOT VERIFIED`） | Repository Governance Owner `蔺鹏` | 当前证据：远端仅 `main`、`20` 个 annotated archive tags、ruleset `21413134` effective、三项 strict checks、无 bypass；这些是配置/快照证据，不是行为验证。关闭需 Reviewer 容量和完整行为证据；空 Cursor suite 仍为 `queued`。 | Frontend governance operational closeout | `2026-08-25` | `BRANCH_PROTECTION`; `FRONTEND main-protection-v1` |
...
| `R-K2-GOV-017` | `ACS-K2-002-GOV-RB1` 要求精确候选独立技术审查、完整检查、thread resolution、无 bypass 和线性 squash merge。Core 的平台 approvals 已由 Owner 精确决定为 `0`，因此 reviews=`[]` 不再是 approval-count blocker；剩余风险是把旧树的审查/检查证据复用于新候选，或在未满足非审批条件时合并。 | 高：K2-002 的 Owner 决定、候选 SHA/tree 与主线接受证据链可能失真；不直接授权媒体、Provider/GPU 或发布 | 中：approval blocker 已移除；治理文档变化将形成新候选，必须重新绑定审查和检查 | 绑定精确候选 SHA/tree；保持 ruleset `20544466` 的五项 strict checks、thread/linear 与 bypass=`[]`；独立技术审查零未解决 blocker/high/medium；完整本地回归和 GitHub Actions 全绿；使用有效 `main` ruleset 的 squash path，并核验合并后 Core `main` SHA/tree/Actions/clean state。 | 缓解中（`MITIGATING / APPROVAL BLOCKER REMOVED / EXACT CANDIDATE REVALIDATION PENDING`） | Project Lead / Repository Governance Owner `蔺鹏` | 当前证据：Core configuration=`CONFORMING / PARTIALLY VERIFIED`；PR #11 platform approval=`NOT REQUIRED / REQUIRED=0`；Repository General settings 仍为 merge/squash/rebase，但有效 `main` ruleset allowed methods=`[squash]`。关闭证据：`ACS-K2-002-GOV-RB1` 第 9 节在同一精确候选上全部满足、无 bypass，并完成 protected squash merge 后核验。 | PR #11 exact candidate + protected merge gate | `2026-08-25` | `ACS-K2-002-GOV-RB1`; `ADR-0014`; `BRANCH_PROTECTION`; `GIT_WORKFLOW 4B`; `R-REPO-GOV-012` |
