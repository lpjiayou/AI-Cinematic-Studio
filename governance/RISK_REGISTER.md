...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `R-CORE-ARCH-001` | M1 AI Director、M3 Script Studio、M5 Series Director 与 Creator Server 曾直接导入 V4 public provider boundary，跳过 V5 相邻层。G1 已将四个活动接触面迁移至 V5，但独立复核确认持续架构守卫未解析 `importlib.import_module` / `__import__` 的导入别名与简单赋值别名，因而别名绕过可能在后续回归中漏检。 | 高：若守卫失效，Application→V4 违规可重新进入仓库并削弱 V5 治理与替换能力；当前生产迁移本身未发现回退 | 中：当前生产导入和动态导入均为零，但已实证守卫可被常见别名形式绕过 | 保留 ADR-0006 和已完成的生产迁移；执行 G1-R1 test-only correction，在唯一 Contract Test 文件中建立 binding-aware AST 守卫，覆盖 `importlib`、`builtins`、`__import__`、导入别名、简单赋值别名和常量 `getattr`，同时证明无关同名函数/方法不误报。完成条件：专项/全量/架构/Git 门禁通过、修复远端验证并由 Project Lead 单独接受。 | 缓解中（`MITIGATING`） | Project Lead / Architecture Owner `蔺鹏` | 当前证据：G1 `0c283eb…` 生产导入 `0`，但 `from importlib import import_module as load; load("services" + ".v4_platform")` 可绕过原守卫；监控触发：任一 `apps/**/*.py` 静态或程序化访问 V4；关闭证据：alias-aware guard 反例/非误报矩阵、生产导入 `0`、完整回归、Remote SHA 等于 Local SHA、Owner Acceptance | `ACS-ARCH-R1-V5-TEXT-GENERATION-G1-R1` Owner Review | `2026-08-13` | `ADR-0006`; `V5_TEXT_GENERATION_CAPABILITY_CONTRACT`; `ACS-ARCH-R1-V5-TEXT-GENERATION-G1-R1-AUTHORIZATION` |
