...
| `R-K2-TRANS-013` | 复用 K2-001 root、数据库、refs、候选或 ADR-0011 例外会把 K2-002 错接到历史验证 lineage；把客户端声明当成 Owner approval，或在无 durable receipt/M5 binding 时执行多域注册，也会生成不可恢复的半套 roots。 | 严重：独立项目 provenance 失真并可能越权 dispatch | 中：现有多个 Operator 明确硬编码 K2-001，正式 durable registration 尚未实现 | 保留 K2-001 原路径作为只读历史；未确认 reviewed-import 必须记录上传、normalized、reviewed 与 canonical content digests，`importedByRef` 只能由认证 service credential 注入且不得表述为已解析的自然人/RBAC。正式确认须解析可信 Owner approval；Project/Series/Episode registration 必须具备 durable receipt、重启幂等、部分写入恢复和 M5 v2 EpisodePlanItem binding；在此之前不得开放 K2-002 multi-domain registration mutation endpoint。 | 缓解中（`MITIGATING`） | Project Lead / Architecture Owner `蔺鹏` | 出现 K2-001 ref、旧数据库路径、名称匹配、transferred exception、客户端自报 approval、无 receipt 多域写入或未绑定 M5 v1 source 即触发；关闭需可信 approval、durable canonical receipt、M5 consumer 成功与 authenticated exact-match。 | K2-002 roots gate | `2026-08-25` | `ADR-0014`; `K2-001 ARCHIVE` |
...
| `R-K2-FORMAT-015` | 旧 K2-001 链固定 16:9、四镜与固定帧预算，直接复用会错误处理 K2-002 的 9:16、EP01 十二镜和逐镜人物/对话约束。 | 高：构图、时长、身份与同步约束失真 | 高：代码审计已复现硬编码 | 以 additive v2 profile、显式 `ShotPlanDraft` 和 authority-read zero-write dynamic preflight 纠正合同并保留 v1 兼容；当前只形成草稿事实并停在 `SCRIPT_VALIDATED`，不生成 `ExecutableShotGraph` 或 `SHOTS_COMPILED`。704→720 延展必须由精确参数 digest 描述。Canonical N-slot append/V4 dispatch 尚未实现，不得声称 E2E 修复。 | 缓解中（`MITIGATING`） | K2 implementation owner / Architecture Owner `蔺鹏` | 任一 1280×720、四镜数组、每镜全人物、无 exact trim，或把 preflight 直接送往 V4 即触发；关闭需 canonical append/dispatch architecture acceptance、targeted + full Core tests。 | K2-002 pre-dispatch chain gate | `2026-08-25` | `ADR-0014`; `ACS-K2-002-GOV-RB1`; `EpisodeProduction v2` |
| `R-K2-GOV-017` | `ACS-K2-002-GOV-RB1` 要求精确候选独立技术审查、完整检查、thread resolution、无 bypass 和线性 squash merge。Core 的平台 approvals 已由 Owner 精确决定为 `0`，因此 reviews=`[]` 不再是 approval-count blocker；剩余风险是把旧树的审查/检查证据复用于后续新候选，或在未满足非审批条件时合并。 | 高：K2-002 的 Owner 决定、候选 SHA/tree 与主线接受证据链可能失真；不直接授权媒体、Provider/GPU 或发布 | 中：Core PR #11 已完成精确候选闭环，但每个后续候选仍必须重新绑定审查和检查 | 绑定每个精确候选 SHA/tree；保持 ruleset `20544466` 的五项 strict checks、thread/linear 与 bypass=`[]`；独立技术审查零未解决 blocker/high/medium；完整本地回归和 GitHub Actions 全绿；使用有效 `main` ruleset 的 squash path，并核验合并后 Core `main` SHA/tree/Actions/clean state。 | 缓解中（`MITIGATING / PR #11 MERGED / PER-CANDIDATE REVALIDATION REQUIRED`） | Project Lead / Repository Governance Owner `蔺鹏` | 当前证据：Core PR #11 已完成精确候选独立技术审查、五项 Actions、conversation resolution 和无 bypass 的受保护 squash merge，并形成 `main@af7f50a8dc7cdccdb7dd47cd425d33a288961cc9`；平台 approval=`NOT REQUIRED / REQUIRED=0`。Repository General settings 仍为 merge/squash/rebase，但有效 `main` ruleset allowed methods=`[squash]`。后续任一候选必须重新满足 `ACS-K2-002-GOV-RB1` 第 9 节，不能复用 PR #11 的 exact-tree 证据。 | Every subsequent exact candidate + protected merge gate | `2026-08-26` | `ACS-K2-002-GOV-RB1`; `ADR-0014`; `BRANCH_PROTECTION`; `GIT_WORKFLOW 4B`; `R-REPO-GOV-012` |
